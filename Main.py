...
window.configure(background='#a0a9ba')
...
def manually_fill():
    ...
    sb.geometry('580x320')
    sb.configure(background='#a0a9ba')
    ...
    def err_screen_for_subject():
        ...
        Label(ec, text='Please enter your subject name!!!', fg='black', bg='snow', font=('times', 16, ' bold ')).pack()
        Button(ec, text='OK', command=ec_delete, fg="black", bg="snow", width=9, height=1, activebackground="Red",
    ...
    def fill_attendance():
        ...
        try:
            ...
            if DB_table_name.lower() not in tables:
                cursor.execute(sql)  ##for create a table
                print(DB_table_name+ " Created")
            else:
                print(DB_table_name+" Already exists")
            connection.commit()
...
'''
        if subb=='':
            ...
            MFW.configure(background='#a0a9ba')
            ...
            def create_csv():
                ...
                csv_name='C:/Users/sheri/Desktop/LambtonCollege/AML 2404 AI and ML Lab/Attendace_management_system-master/Attendance/Manually Attendance/'+DB_table_name+'.csv'
            ...
            def attf():
                import subprocess
                subprocess.Popen(r'explorer /select,"C:\Users\sheri\Desktop\LambtonCollege\AML 2404 AI and ML Lab\Attendace_management_system-master\Attendance\Manually Attendance\"')
    ...
    SUB = tk.Label(sb, text="Enter Subject", width=15, height=2, fg="black", bg="snow", font=('times', 15, ' bold '))
    ...
    fill_manual_attendance = tk.Button(sb, text="Fill Attendance",command=fill_attendance, fg="black", bg="snow", width=20, height=2,
                       activebackground="Red", font=('times', 15, ' bold '))
...
def person_present():
    ...
    Label(sc1, text='Images already trained', fg='red', bg='snow', font=('times', 16, ' bold ')).pack(ipadx = 5, ipady = 5)
    Button(sc1, text='OK', command=del_sc1, fg="black", bg="snow", width=9, height=1, activebackground = "Red" ,font=('times', 15, ' bold ')).place(x=90,y= 50)
...
def take_img():
    ...
    if l1 == '':
        ...
        try:
            ...
            Name = txt2.get()
            person_name = Name
            if os.path.exists("TrainingImage/" + person_name + "." + Enrollment):
                person_present()
                return
            sampleNum = 0
            while (True):
                ...
                for (x, y, w, h) in faces:
                    ...
                    if not os.path.exists("TrainingImage/" + person_name + "." + Enrollment):
                        os.makedirs("TrainingImage/" + person_name + "." + Enrollment)
                    # saving the captured face in the dataset folder
                    cv2.imwrite("TrainingImage/" + person_name + "." + Enrollment + "/" + Name + "." + Enrollment + '.' + str(sampleNum) + ".jpg",
                        gray[y:y + h, x:x + w])
            ...
            if not (student_df['Enrollment'] != Enrollment).any():
                student_df = student_df.append(row, ignore_index=True)
            else:
                print('Student already registered for Enrollment: ', Enrollment)
            print(student_df)
...
def subjectchoose():
        ...
        if time.time() < future:
            if sub == '':
                ...
                try:
                    ...
                    cursor.execute(check_tables)
                    tables = [row[0] for row in cursor.fetchall()]
                    if str(Subject).lower() not in tables:
                        cursor.execute(sql)  ##for create a table
                ...
                root.configure(background='snow')
                cs = 'C:/Users/sheri/Desktop/LambtonCollege/AML 2404 AI and ML Lab/Attendace_management_system-master/' + fileName
            root.mainloop()
    ...
    windo.configure(background='#a0a9ba')
    Notifica = tk.Label(windo, text="Attendance filled Successfully", bg="snow", fg="white", width=33,
                            height=2, font=('times', 15, 'bold'))

    def Attf():
        import subprocess
        subprocess.Popen(r'explorer /select,"C:\Users\sheri\Desktop\LambtonCollege\AML 2404 AI and ML Lab\Attendace_management_system-master\Attendance\"')

    attf = tk.Button(windo,  text="Check Sheets",command=Attf,fg="black"  ,bg="snow"  ,width=12  ,height=1 ,activebackground = "Red" ,font=('times', 14, ' bold '))
    attf.place(x=430, y=255)

    sub = tk.Label(windo, text="Enter Subject", width=15, height=2, fg="black", bg="snow", font=('times', 15, ' bold '))
    sub.place(x=30, y=100)

    options = ["Advanced_Python", "AI_ML_Lab", "Visualization", "NLP", "Data Mining"]
    selection_option = tk.StringVar(windo)
    selection_option.set(options[0])

    tx = tk.Entry(windo, width=20, textvariable=selection_option, bg="snow", fg="black", font=('times', 23, ' bold '))
    tx.place(x=250, y=105)
    option_menu = tk.OptionMenu(windo, selection_option, *options)
    option_menu.config(width=20, font=("times", 23, "bold"), bg="snow", fg="black")
    option_menu.pack()

    fill_a = tk.Button(windo, text="Fill Attendance", fg="black", command=Fillattendances, bg="snow", width=20, height=2,
                       activebackground="Red", font=('times', 15, ' bold '))
    ...
    def log_in():
        ...
        if username == 'admin' :
            if password == 'admin':
                ...
                cs = 'C:/Users/sheri/Desktop/LambtonCollege/AML 2404 AI and ML Lab/Attendace_management_system-master/StudentDetails/StudentDetails.csv'
                with open(cs, newline="") as file:
                    reader = csv.reader(file)
                    r = 0
...
message = tk.Label(window, text="Facial Recognition- Attendance Management System", bg="white", fg="black", width=50,
                   height=3, font=('Arial', 30, 'italic bold'), relief = "raised", pady=10).pack(ipadx=10)

Notification = tk.Label(window, text="All things good", bg="Green", fg="white", width=15,
                      height=3, font=('times', 17, 'bold'))

lbl = tk.Label(window, text="Enter Enrollment ID", width=20, height=2, fg="black", bg="snow", font=('times', 15, ' bold '))
...
def testVal(inStr,acttyp):
    if acttyp == '1': #insert
        if not inStr.isdigit():
            return False
    return True
txt = tk.Entry(window, validate="key", width=20, bg="snow", fg="black", font=('times', 25, ' bold '))
txt['validatecommand'] = (txt.register(testVal),'%P','%d')
txt.place(x=550, y=210)

lbl2 = tk.Label(window, text="Enter Your Name", width=20, fg="black", bg="snow", height=2, font=('times', 15, 'bold'))
lbl2.place(x=200, y=300)

txt2 = tk.Entry(window, width=20, bg="snow", fg="black", font=('times', 25, 'bold'))
txt2.place(x=550, y=310)

clearButton = tk.Button(window, text="Clear", command=clear,fg="black", bg="snow", width=10, height=1, activebackground = "Black", font=('times', 15, 'bold'))
clearButton.place(x=950, y=210)

clearButton1 = tk.Button(window, text="Clear",command=clear1,fg="black", bg="snow"  ,width=10 ,height=1, activebackground = "Black" ,font=('times', 15, ' bold '))
clearButton1.place(x=950, y=310)

AP = tk.Button(window, text="Check Register students", command=admin_panel, fg="black", bg="snow", width=19 ,height=1, activebackground = "Black", font=('times', 15, ' bold '))
AP.place(x=990, y=410)

takeImg = tk.Button(window, text="Take Images", command=take_img, fg="black", bg="snow", width=20, height=3, activebackground = "Black", font=('times', 15, ' bold '))
takeImg.place(x=90, y=500)

trainImg = tk.Button(window, text="Train Images",fg="black",command=trainimg ,bg="snow"  ,width=20  ,height=3, activebackground = "Black" ,font=('times', 15, ' bold '))
trainImg.place(x=390, y=500)

FA = tk.Button(window, text="Automatic Attendance", fg="black", command=subjectchoose, bg="snow", width=20, height=3, activebackground = "Black", font=('times', 15, ' bold '))
FA.place(x=690, y=500)

quitWindow = tk.Button(window, text="Manually Fill Attendance", command=manually_fill, fg="black", bg="snow", width=20  ,height=3, activebackground = "Black", font=('times', 15, ' bold '))
